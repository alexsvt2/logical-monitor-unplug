...
enum MonitorPolicy {
    ...
    static func validate(_ monitor: Monitor, enable: Bool, monitors: [Monitor]) throws {
        guard monitor.available else {
            throw MonitorError(message: L("error.unavailable"))
        }
        if !enable {
            guard !monitor.builtin else { throw MonitorError(message: L("error.builtin")) }
            guard !monitor.main else { throw MonitorError(message: L("error.primary")) }
            guard monitors.contains(where: { $0.uuid != monitor.uuid && $0.active && $0.main }) else {
                throw MonitorError(message: L("error.single"))
            }
        }
    }
}
...
final class MonitorController {
    ...
    func monitors() throws -> [Monitor] {
        ...
        guard CGGetOnlineDisplayList(UInt32(ids.count), &ids, &count) == .success else {
            throw MonitorError(message: L("error.list"))
        }
        ...
        for id in ids.prefix(Int(count)) {
            ...
            let name = screen?.localizedName ?? (remembered?.uuid == key ? remembered?.name : nil)
                ?? L("display.fallback", result.count + 1)
    ...
    func recover() throws -> String {
        ...
        guard fd >= 0 else { throw MonitorError(message: L("error.lock")) }
        defer { Darwin.close(fd) }
        guard flock(fd, LOCK_EX | LOCK_NB) == 0 else { throw MonitorError(message: L("error.busy")) }
        ...
        guard let handle = dlopen("/System/Library/Frameworks/CoreGraphics.framework/CoreGraphics", RTLD_LAZY) else {
            throw MonitorError(message: L("error.load"))
        }
        defer { dlclose(handle) }
        guard let symbol = dlsym(handle, "CGSConfigureDisplayEnabled") else {
            throw MonitorError(message: L("error.recoveryUnavailable"))
        }
        ...
        for _ in 0..<20 {
            ...
            if !recovered.isEmpty {
                return L("recovery.success", recovered.map { $0.name }.joined(separator: ", "))
            }
        }
        throw MonitorError(message: L("error.recoveryNoDisplay"))
    }
    ...
    func change(uuid key: String, enable: Bool) throws -> String {
        ...
        guard let monitor = all.first(where: { $0.uuid == key }) else { throw MonitorError(message: L("error.targetMissing")) }
        try MonitorPolicy.validate(monitor, enable: enable, monitors: all)
        if monitor.active == enable { return enable ? L("result.alreadyEnabled") : L("result.alreadyDisabled") }
        try select(monitor)
        ...
        guard let handle = handle else { throw MonitorError(message: L("error.load")) }
        defer { dlclose(handle) }
        guard let symbol = dlsym(handle, "CGSConfigureDisplayEnabled") else {
            throw MonitorError(message: L("error.apiUnavailable"))
        }
        typealias Configure = @convention(c) (OpaquePointer?, UInt32, Bool) -> Int32
        let configure = unsafeBitCast(symbol, to: Configure.self)
        var config: CGDisplayConfigRef?
        guard CGBeginDisplayConfiguration(&config) == .success else { throw MonitorError(message: L("error.begin")) }
        let error = configure(config, monitor.id, enable)
        guard error == 0 else {
            CGCancelDisplayConfiguration(config)
            throw MonitorError(message: L("error.rejected", error))
        }
        guard CGCompleteDisplayConfiguration(config, .forSession) == .success else {
            throw MonitorError(message: L("error.apply"))
        }
        ...
        for _ in 0..<20 {
            ...
            if enable ? (current != 0 && enabled(current)) : (current == 0 || !enabled(current)) {
                return enable ? L("result.enabled") : L("result.disabled")
            }
        }
        throw MonitorError(message: L("error.verify"))
    }
}
